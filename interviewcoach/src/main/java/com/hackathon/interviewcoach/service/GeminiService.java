...
public class GeminiService {
    ...
    public List<String> generateQuestions(String prompt, int expectedCount) {
        ...
        try {
            ...
            log.error("Failed to parse Gemini question generation response", e);
    ...
    public EvaluationResult evaluateAnswer(String prompt) {
        ...
        try {
            ...
            return new EvaluationResult(
                    ...
                    node.path("topicsToRevise").asText(""),
                    node.path("feedback").asText("")
            );

        } catch (Exception e) {
            log.error("Failed to parse Gemini evaluation response", e);
            throw new BadRequestException("Failed to evaluate answer.");
        }
    }
    ...
    private String extractJson(String text, char openChar, char closeChar) {
        ...
        if (start == -1 || end == -1 || end < start) {
            ...
            Pattern pattern = Pattern.compile(
                    Pattern.quote(String.valueOf(openChar))
                            + ".*"
                            + Pattern.quote(String.valueOf(closeChar)),
                    Pattern.DOTALL
            );
